...
This sample builds against **AEP 0.5.0**, declared as `aep.version` in `../gradle.properties`.
These three files belong here, committed to the repository:

| File | Component | Coordinate (internal use only) |
|---|---|---|
| `AEP.Core-0.5.0.jar` | Core | `com.ambokit.aep:aep-core` |
| `AEP.HostSDK-0.5.0.aar` | Host SDK | `com.ambokit.aep:aep-host-sdk` |
| `AEP.AndroidTV.Adapter-0.5.0.aar` | Android TV adapter | `com.ambokit.aep:aep-androidtv-adapter` |
...
```bash
gh release download v0.5.0 --repo ssriramiyer29/amboexperienceplatform --dir .
```
